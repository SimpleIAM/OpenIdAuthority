    ...
    public static class AppConfigHelper
    {
        ...
        public static Client GetClientFromAppConfig(AppConfig config)
        {
            ...
            var client = new Client()
            {
                ...
                RedirectUris = config.RedirectUris?.Length > 0 ? config.RedirectUris : new string[] { baseUrl, $"{baseUrl}/signin-oidc" },
                FrontChannelLogoutUri = config.FrontChannelLogoutUri ?? $"{baseUrl}/signout-oidc",
                PostLogoutRedirectUris = config.PostLogoutRedirectUris?.Length > 0 ? config.PostLogoutRedirectUris : new string[] { baseUrl, $"{baseUrl}/", $"{baseUrl}/signout-callback-oidc" },
